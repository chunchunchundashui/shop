...
<button type="button" tooltip="添加品牌"  class="btn btn-sm btn-azure btn-addon" onClick="javascript:window.location.href = '<?php echo url('alternate_img/add'); ?>'"> <i class="fa fa-plus"></i> Add
</button>
<div class="row">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <?php if(is_array($lst) || $lst instanceof \think\Collection || $lst instanceof \think\Paginator): $i = 0; $__LIST__ = $lst;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                             <tr>
                                ...
                                <td align="center"><input type="text" value="<?php echo $vo['sort']; ?>" name="sort[<?php echo $vo['id']; ?>]" style="width: 50px; text-align: center"></td>
                               <td align="center"><?php echo $vo['title']; ?></td>
                                <td><a href="<?php echo $vo['link_url']; ?>" target="_blank"><?php echo $vo['link_url']; ?></a></td>
                                <td align="center">
                                    <?php if($vo['img_src'] != ''): ?>
                                    <img src="/shop/public/static/uploads/<?php echo $vo['img_src']; ?>" height="30px" alt="">
                                    <?php else: ?>
                                    暂无图片
                                    <?php endif; ?>
                                </td>
                                ...
                                <td align="center">
                                    <a href="<?php echo url('admin/alternate_img/edit', ['id' => $vo['id']]); ?>" class="btn btn-primary btn-sm shiny">编辑</a>
                                    <a href="javascript:;" class="btn btn-danger alternate_img_del btn-sm shiny" onclick="del(this, '<?php echo $vo['id']; ?>')">删除</a>
                                </td>
                            </tr>
    ...
    <script>
      function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
          ...
          //发异步删除数据
          $.post("<?php echo url('alternate_img/del'); ?>",{id:id},function (data) {
            if(data.code == 1){
              div.remove();
              //删除成功 删除元素
