...
<button type="button" tooltip="添加品牌"  class="btn btn-sm btn-azure btn-addon" onClick="javascript:window.location.href = '<?php echo url('user/add'); ?>'"> <i class="fa fa-plus"></i> Add
</button>
<div class="row">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <?php if(is_array($lst) || $lst instanceof \think\Collection || $lst instanceof \think\Paginator): $i = 0; $__LIST__ = $lst;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                             <tr>
                                ...
                                <td align="center"><?php if($vo['email']): ?><?php echo $vo['email']; else: ?>暂未填写<?php endif; ?></td>
                                <td align="center"><?php if($vo['mobile_phone'] == ''): ?>暂未填写<?php else: ?><?php echo $vo['mobile_phone']; endif; ?></td>
                                <td align="center"><?php if($vo['register_type'] == 1): ?>手机验证<?php elseif($vo['register_type'] == 2): ?>手机邮箱都验证<?php else: ?>邮箱验证<?php endif; ?></td>
                                <td align="center"><?php echo $vo['points']; ?></td>
                                <td align="center"><?php echo $vo['level_name']; ?></td>
                                <td align="center">
                                    <a href="<?php echo url('admin/user/edit', ['id' => $vo['id']]); ?>" class="btn btn-primary btn-sm shiny">编辑</a>
                                    <a href="javascript:;" class="btn btn-danger user_del btn-sm shiny" onclick="del(this, '<?php echo $vo['id']; ?>')">删除</a>
                                </td>
                            </tr>
    ...
    <script>
      function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
          ...
          //发异步删除数据
          $.post("<?php echo url('user/del'); ?>",{id:id},function (data) {
            if(data.code == 1){
              div.remove();
              //删除成功 删除元素
