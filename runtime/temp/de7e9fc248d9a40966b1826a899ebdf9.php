...
<button type="button" tooltip="添加品牌"  class="btn btn-sm btn-azure btn-addon" onClick="javascript:window.location.href = '<?php echo url('CategoryBrands/add'); ?>'"> <i class="fa fa-plus"></i> Add
</button>
<div class="row">
                    ...
                    <table class="table table-bordered table-hover">
                        <thead class="">
                            <tr>
                                ...
                                <th class="text-center">关联品牌名称</th>
                                <th class="text-center">关联栏目</th>
                                <th class="text-center">推广图</th>
                                <th class="text-center">推广图地址</th>
                                <th class="text-center" width="14%">操作</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php if(is_array($cbRes) || $cbRes instanceof \think\Collection || $cbRes instanceof \think\Paginator): $i = 0; $__LIST__ = $cbRes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                             <tr>
                                <td align="center"><?php echo $vo['id']; ?></td>
                                <td align="center"><?php if($vo['brand_name'] != ''): ?><?php echo $vo['brand_name']; else: ?> <h style="color: red;">无关联品牌</h> <?php endif; ?></td>
                                <td align="center"><?php echo $vo['cate_name']; ?></td>
                                <td align="center">
                                    <?php if($vo['pro_img'] != ''): ?>
                                    <img src="/shop/public/static/pro_img/<?php echo $vo['pro_img']; ?>" height="30px" alt="">
                                    <?php else: ?>
                                    暂无图片
                                    <?php endif; ?>
                                </td>
                                <td align="center"><?php if($vo['pro_url'] != ''): ?><a href="<?php echo $vo['pro_url']; ?>" target="_blank"><?php echo $vo['pro_url']; ?></a><?php else: ?><h style="color: red;">无跳转地址</h><?php endif; ?></td>
                                <td align="center">
                                    <a href="<?php echo url('admin/categoryBrands/edit', ['id' => $vo['id']]); ?>" class="btn btn-primary btn-sm shiny">编辑</a>
    ...
    <script>
      function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
          ...
          //发异步删除数据
          $.post("<?php echo url('CategoryBrands/del'); ?>",{id:id},function (data) {
            if(data.code == 1){
              div.remove();
              //删除成功 删除元素
