...
<button type="button" tooltip="添加左图"  class="btn btn-sm btn-azure btn-addon" onClick="javascript:window.location.href = '<?php echo url('CategoryAd/add'); ?>'"> <i class="fa fa-plus"></i> Add
</button>
<div class="row">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <?php if(is_array($caRes) || $caRes instanceof \think\Collection || $caRes instanceof \think\Paginator): $i = 0; $__LIST__ = $caRes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ca): $mod = ($i % 2 );++$i;?>
                             <tr>
                                ...
                                <td align="center"><?php echo $ca['position']; ?></td>
                                 <td align="center">
                                 <?php if($ca['img_src'] != ''): ?>
                                 <img src="/shop/public/static/pro_img/<?php echo $ca['img_src']; ?>" height="30px" alt="">
                                 <?php else: ?>
                                 暂无左图
                                 <?php endif; ?>
                               </td>
                               <td align="center"><?php echo $ca['cate_name']; ?></td>
                                <td align="center"><?php if($ca['link_url'] != ''): ?><a href="<?php echo $ca['link_url']; ?>" target="_blank"><?php echo $ca['link_url']; ?></a><?php else: ?><h style="color: red;">无跳转地址</h><?php endif; ?></td>
                                <td align="center">
                                    <a href="<?php echo url('admin/categoryAd/edit', ['id' => $ca['id']]); ?>" class="btn btn-primary btn-sm shiny">编辑</a>
                                    <a href="javascript:;" class="btn btn-danger brand_del btn-sm shiny" onclick="del(this,'<?php echo $ca['id']; ?>')">删除</a>
                                </td>
                            </tr>
    ...
    <script>
      function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
          ...
          //发异步删除数据
          $.post("<?php echo url('CategoryAd/del'); ?>",{id:id},function (data) {
            if(data.code == 1){
              div.remove();
              //删除成功 删除元素
