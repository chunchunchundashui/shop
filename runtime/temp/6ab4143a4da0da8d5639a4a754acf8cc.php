<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:58:"D:\phpStudy\WWW\shop/application/index\view\flow\flow4.htm";i:1599661676;s:61:"D:\phpStudy\WWW\shop\application\index\view\common\footer.htm";i:1592318019;}*/ ?>
<!doctype html>
<html>
<head>
...
<div class="header header-cart">
  <div class="w w1200">
    <div class="logo">
      <div class="logoImg"><a href="/"><img src="/shop/public/static/index/img/logo.gif"/></a></div>
    ...
    <div class="payOrder-warp" id="pay_main">
      <div class="payOrder-desc">
        ...
        <div class="pay-txt">
          ...
          <p>您选定的支付方式为：<span id="paymode"><?php if($orderInfo['payment'] == 1): ?> 支付宝 <?php elseif($orderInfo['payment'] == 2): ?> 微信 <?php else: ?> 余额 <?php endif; ?></span>
          </p>
          <p>配送方式: <span id="express"><?php echo $orderInfo['distribution']; ?></span></p>
        </div>
        <div class="o-list">
          <div class="o-list-info">
            <span id="shdz">寄送至：<?php echo $orderInfo['province']; ?>  <?php echo $orderInfo['city']; ?>  <?php echo $orderInfo['county']; ?>  &nbsp;&nbsp;<?php echo $orderInfo['adress']; ?></span>
        ...
        <?php if($orderInfo['payment'] == 1): ?>
          <?php echo $payBtn; else: if($orderInfo['pay_status'] == 0): ?>
              <img id="ewm" width="100px" src="<?php echo url('index/Flow/wxewm', array('outTradeNo' => $orderInfo['out_trade_no']),''); ?>" alt="">
            <?php else: ?>
              <img id="ewm" width="100px" src="http://shop.yccpanda.com/public/static/index/img/wxpay.png" alt="">
            <?php endif; endif; ?>
      </div>
    </div>
  </div>
...
<script type="text/javascript" src="/shop/public/static/index//shop/public/static/index/js/perfect-scrollbar.min.js"></script>
...
<script type="text/javascript">
  //微信支付异步获取支付状态更改二维码
  var interval = setInterval("my_monitor()", 1000);
  function my_monitor(){
    var out_trade_no = <?php echo $orderInfo['out_trade_no']; ?>;
    $.ajax({
      type: 'post',
      url: "<?php echo url('index/Flow/wxPayStatus'); ?>",
      data: {out_trade_no:out_trade_no},
      dataType:'json',
      success:function(res) {
        if (res.pay_status == 1) {
          $("#ewm").attr('src',"http://localhost/shop/index.php/static/index/img/wxpay.png");
