...
class Notify extends Base
{
    ...
    public function checkPrice($arr){
        if($arr['return_code'] == 'SUCCESS' && $arr['result_code'] == 'SUCCESS'){
            $orderTradePrice = db('order')->where('out_trade_no',$arr['out_trade_no'])->value('order_total_price');
            $orderTradePrice = $orderTradePrice*100;
            if($arr['total_fee'] == $orderTradePrice){//生产环境需要根据订单号在数据库中查询金额
                return true;
            }else{
